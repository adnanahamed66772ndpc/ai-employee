...
import type { LlmProxyDeps, PriceBook } from "./llm.ts";
...
describe("spending settings", () => {
  it("defaults to a $2 day and deepseek-v4-pro, validates, and saves both", async () => {
    const call = makeApp();
    expect(await (await call("/api/settings/spending")).json()).toMatchObject({ dailyBudgetUsd: 2, escalationModel: "deepseek-v4-pro", spentTodayUsd: 0 });
    ...
    expect((await put({ dailyBudgetUsd: 5, escalationModel: "rm -rf /" })).status).toBe(400);
    expect(await (await put({ dailyBudgetUsd: null, escalationModel: "glm-5.3" })).json()).toMatchObject({ dailyBudgetUsd: null, escalationModel: "glm-5.3" });
